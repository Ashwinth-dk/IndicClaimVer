...
export default function PipelineControl() {
  ...
  const handleStartCrawl = async (e) => {
    ...
    try {
      await startCrawlPipeline({
        ...
        auto_train: true, // Backend automatically trains if Quality Gate passes
      ...
      setErrorMsg(err.message || 'Failed to start automated crawl');
    } finally {
      setLoading(false);
    }
  };
  ...
  const crawlResult = statusData?.crawl_result || {};
  ...
  // Compute stage status index (0 to 6)
  const stageIndexMap = {
    QUEUED: 0,
    CRAWLING: 0,
    PROCESSING: 1,
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto p-4 sm:p-6 text-slate-100 font-sans">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-5 rounded-2xl bg-gradient-to-r from-blue-950/40 via-dark-900 to-indigo-950/40 border border-blue-900/40 shadow-xl">
        <div className="space-y-1">
          <div className="flex items-center gap-2.5">
            <div className="p-2 rounded-xl bg-blue-600/20 border border-blue-500/30 text-blue-400 shadow-glow-blue">
              <Sparkles className="w-5 h-5" />
            </div>
            <h1 className="text-xl sm:text-2xl font-black font-mono tracking-tight text-white">
              Automated Crawl & Model Pipeline
            </h1>
          </div>
          <p className="text-xs sm:text-sm text-slate-400">
            Single-Action Pipeline: START CRAWL → Extract & Deduplicate → Quality Gate → Auto MuRIL Fine-Tuning → Activation
          </p>
        </div>

        {/* Active Model Badge */}
        <div className="flex items-center gap-3 bg-dark-950/80 px-4 py-2.5 rounded-xl border border-slate-800">
          <Cpu className="w-4 h-4 text-emerald-400" />
          <div className="text-xs">
            <span className="text-slate-400 block text-[10px] uppercase font-mono tracking-wider">Active Prediction Model</span>
        ...
        <form onSubmit={handleStartCrawl} className="space-y-4">
          <div className="space-y-2">
            <label className="text-xs font-mono font-semibold uppercase tracking-wider text-slate-300 flex items-center justify-between">
              ...
              <span className="text-[11px] text-blue-400 normal-case font-sans">Single click starts complete end-to-end pipeline</span>
            </label>
          ...
          <div>
            ...
            >
              <span>{showAdvanced ? '▼ Hide Advanced Pipeline Settings' : '▶ Show Advanced Pipeline Settings'}</span>
            </button>

            {showAdvanced && (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-3 mt-2 border-t border-slate-800">
                <div className="space-y-1">
                  <label className="text-[11px] font-mono text-slate-400">Target Examples Count</label>
                  <input
                    type="number"
                    min="10"
                    max="2000"
                ...
                <div className="space-y-1">
                  <label className="text-[11px] font-mono text-slate-400">Fine-Tuning Epochs (If Quality Gate Passes)</label>
                  <input
                    type="number"
                    min="1"
                    max="10"
      ...
      <div className="p-5 rounded-2xl bg-dark-900/70 border border-slate-800/80 space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-7 gap-2.5">
          {PIPELINE_STAGES.map((st, idx) => {
            // Check if stage is completed, active, skipped, or pending
            let isCompleted = false;
            let isActive = false;
            let isSkipped = false;

            if (idx <= 3) {
              // Crawling, Processing, Dataset Building, Quality Validation
              ...
              // Stages 4, 5, 6 (Training, Evaluation, Activation)
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        <div className="p-4 rounded-xl bg-dark-900/60 border border-slate-800/80 space-y-1">
          ...
          <div className="text-xl font-bold font-mono text-blue-400">
            {crawlStatus.urls_crawled ?? crawlResult.urls_crawled ?? 0}
          </div>
        </div>

        <div className="p-4 rounded-xl bg-dark-900/60 border border-slate-800/80 space-y-1">
          <div className="text-[11px] font-mono text-slate-400 uppercase">Evidence Extracted</div>
          <div className="text-xl font-bold font-mono text-indigo-400">
            {crawlStatus.evidence_candidates ?? crawlResult.collected ?? 0}
          </div>
        </div>

        <div className="p-4 rounded-xl bg-dark-900/60 border border-slate-800/80 space-y-1">
          <div className="text-[11px] font-mono text-slate-400 uppercase">Training Examples</div>
          <div className="text-xl font-bold font-mono text-teal-400">
            {crawlStatus.collected_count ?? crawlResult.collected ?? 0}
          </div>
        </div>

        <div className="p-4 rounded-xl bg-dark-900/60 border border-slate-800/80 space-y-1">
          <div className="text-[11px] font-mono text-slate-400 uppercase">Supports / Refutes</div>
          <div className="text-sm font-bold font-mono flex items-center gap-1.5 pt-1">
            <span className="text-emerald-400">S:{crawlStatus.supports_count ?? crawlResult.supports ?? 0}</span>
            <span className="text-slate-400">/</span>
            <span className="text-rose-400">R:{crawlStatus.refutes_count ?? crawlResult.refutes ?? 0}</span>
          </div>
        </div>
        ...
        <div className="p-4 rounded-xl bg-dark-900/60 border border-slate-800/80 space-y-1">
          ...
          <div className="text-xl font-bold font-mono text-amber-400">
            {crawlStatus.rejected_count ?? crawlResult.rejected ?? 0}
          </div>
        </div>
      ...
      {/* Model Evaluation & Training Metrics (if available) */}
      {trainResult?.metrics && (
        <div className="p-5 rounded-2xl bg-gradient-to-r from-emerald-950/20 via-dark-900 to-teal-950/20 border border-emerald-500/30 space-y-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 font-mono text-xs uppercase tracking-wider text-emerald-300 font-bold">
              <TrendingUp className="w-4 h-4 text-emerald-400" />
              <span>Model Evaluation Results ({trainResult.version || activeModel?.active_version || 'Latest'})</span>
            </div>
            <span className="px-2.5 py-0.5 rounded-full text-[11px] font-mono bg-emerald-500/20 text-emerald-300 border border-emerald-500/40">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs font-mono">
            <div className="p-3 rounded-lg bg-dark-950/80 border border-slate-800">
              ...
              <span className="text-base font-bold text-emerald-400">
                {Math.round((trainResult.metrics.test_accuracy || 0) * 100)}%
              </span>
            </div>
            <div className="p-3 rounded-lg bg-dark-950/80 border border-slate-800">
              <span className="text-slate-400 block text-[10px]">Macro F1 Score</span>
              <span className="text-base font-bold text-blue-400">
                {trainResult.metrics.macro_f1 ?? 'N/A'}
              </span>
            </div>
            <div className="p-3 rounded-lg bg-dark-950/80 border border-slate-800">
              <span className="text-slate-400 block text-[10px]">SUPPORTS F1</span>
              <span className="text-base font-bold text-teal-400">
                {trainResult.metrics.supports_f1 ?? 'N/A'}
              </span>
            </div>
            <div className="p-3 rounded-lg bg-dark-950/80 border border-slate-800">
              <span className="text-slate-400 block text-[10px]">REFUTES F1</span>
              <span className="text-base font-bold text-rose-400">
                {trainResult.metrics.refutes_f1 ?? 'N/A'}
              </span>
            </div>
          </div>
        </div>
      ...
      {/* Past Run History */}
      {history.length > 0 && (
        <div className="p-5 rounded-2xl bg-dark-900/50 border border-slate-800/80 space-y-3">
          <div className="text-xs font-mono uppercase tracking-wider text-slate-300 font-semibold flex items-center gap-2">
            <Clock className="w-3.5 h-3.5 text-indigo-400" />
            <span>Recent Pipeline Runs ({history.length})</span>
          </div>
          ...
          <div className="space-y-2 max-h-56 overflow-y-auto pr-1">
            {history.map((run, i) => (
              <div
                key={i}
                className="p-3 rounded-xl bg-dark-950/80 border border-slate-800 flex items-center justify-between text-xs font-mono"
              >
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <span className="font-bold text-slate-200">Job: {run.job_id}</span>
                    <span className={`px-2 py-0.5 rounded text-[10px] ${
                      run.stage === 'COMPLETED' ? 'bg-emerald-950/60 text-emerald-300 border border-emerald-500/30' :
                      run.stage === 'FAILED' ? 'bg-rose-950/60 text-rose-300 border border-rose-500/30' :
                      'bg-slate-800 text-slate-300'
                    }`}>
                      {run.stage}
                    </span>
                  </div>
                  <p className="text-[11px] text-slate-400 font-sans truncate max-w-md">{run.stage_details}</p>
                </div>
